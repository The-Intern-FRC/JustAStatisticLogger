...
public class EgoCalculatorFinal {
    ...
    static {
        BADGE_DEFINITIONS.put("Mini but Mighty", new Badge("Mini but Mighty", "Ego < 250", "Small but packs a punch. Who knew?"));
        BADGE_DEFINITIONS.put("Rising Star", new Badge("Rising Star", "250 ≤ Ego < 300", "Modestly competent. Shine, little one."));
        BADGE_DEFINITIONS.put("Confident Programmer", new Badge("Confident Programmer", "300 ≤ Ego < 350", "Swagger in code. Not cocky… okay, maybe a little."));
        BADGE_DEFINITIONS.put("Sky High Ego", new Badge("Sky High Ego", "350 ≤ Ego < 400", "Tall enough to see over your ego… barely."));
        BADGE_DEFINITIONS.put("Overachiever", new Badge("Overachiever", "Ego ≥ 450", "Ego > 450. Maybe tone it down before you break something."));
        BADGE_DEFINITIONS.put("Napoleon Complex", new Badge("Napoleon Complex", "Name = Napoleon", "The ego makes up the height difference."));
        BADGE_DEFINITIONS.put("The Intern", new Badge("The Intern", "Name = Intern & Ego = 382.63", "There Can Only Be One."));
        ...
        BADGE_DEFINITIONS.put("How Did We End Up Here?", new Badge("How Did We End Up Here?", "Viewed calculation", "You wanted to see how the score is calculated."));
        BADGE_DEFINITIONS.put("Egomaniac", new Badge("Egomaniac", "Calculated ego 10 times", "Calculated ego 10 times. Stop flexing."));
    }
    ...
    private static void calculateEgo(String name) {
        ...
        double ego = 5.892 * heightInches - 6.243;
        System.out.printf("\nYour Ego Score: %.2f\n", ego);
        ...
        else if (ego < 350) System.out.println("Decent. Not enough to break anything, though.");
        else if (ego < 450) System.out.println("Careful, your ego is starting to rival your height.");
        else System.out.println("Wow, ego over 450. Tone it down before someone trips over your confidence.");

        System.out.print("\nView how your score is calculated? (y/n): ");
        String viewCalc = SCANNER.nextLine().trim().toLowerCase();
        if (viewCalc.equals("y")) {
            showScience(heightInches, ego);
            triggerBadge(name, "How Did We End Up Here?");
        }

        System.out.print("Save this score to leaderboard? (y/n): ");
        boolean save = SCANNER.nextLine().trim().equalsIgnoreCase("y");
        if (!save) triggerBadge(name, "Self-Conscious");
    ...
    private static void checkBadges(String name, double ego) {
        ...
        // Range badges
        if (ego < 250) triggerBadge(name, "Mini but Mighty");
        else if (ego < 300) triggerBadge(name, "Rising Star");
        else if (ego < 350) triggerBadge(name, "Confident Programmer");
        else if (ego < 400) triggerBadge(name, "Sky High Ego");
        else if (ego >= 450) triggerBadge(name, "Overachiever");
    }

    private static void triggerBadge(String name, String badgeTitle) {
        Set<String> earned = loadUserBadges(name);
        if (earned.contains(badgeTitle)) return; // cannot earn twice
    ...
    private static void viewProfile(String name) {
        ...
        if (!scores.isEmpty()) {
            System.out.printf("Lowest Ego: %.2f | Highest Ego: %.2f | Average Ego: %.2f\n",
